// The migration walk a real client brain has to survive: schema 22 -> 44.
//
...
// with a POPULATED database. Migrations 0023..0044 have
...
check("today's tree has 0023..0044 pending", PENDING.length === 22 && PENDING.at(-1).version === 44,
...
console.log("\n--- the walk: apply 0023..0044 to that populated brain ---");
const applied = [];
const errors = [];
...
check("all 22 pending migrations apply to a populated schema-22 brain",
...
if (errors.length === 0) {
  ...
  console.log("\n--- every declared object of 0023..0044 exists after the walk ---");
  ...
  for (const migration of PENDING) {
    ...
    const declared = [...declaredObjects.values(), ...declaredColumns];
  ...
  const upsertError = probe(() => {
    ...
    ).run("fixture-brain", "0.4.0", 44, 0, new Date().toISOString(), "stable");
    return null;
  }, "threw");
  ...
  check("migrate records schema_version 44", state?.schema_version === 44, JSON.stringify(state?.schema_version));
  ...
  check("schema_migrations is contiguous 1..44 after the upgrade",
    versions.length === 44 && versions.every((v, i) => v === i + 1), JSON.stringify(versions));
}
...
console.log("\n--- 0033's DROP/CREATE window: the hazard the writer barrier exists to close ---");
{
  ...
  check("cmdMigrate lists 33 and 44 among the migrations requiring verified writer quiescence",
    /writerQuiescenceMigrations = new Set\(\[10, 11, 12, 13, 33, 44\]\)/
...
// WHOLE file on the next `brain migrate`. Every statement in 0023..0044 must
