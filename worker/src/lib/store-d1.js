...
function sourceOwnerRemedy(source, concern = "refresh") {
  ...
  const full = {
    ...
    imessage: "On the owner Mac, confirm Full Disk Access, then run iMessage with --reset and no --limit. That records a complete sweep of the selected local Messages database end to end, and nothing beyond it. A conversation still in progress is held rather than cut, so the sweep's date range ends at the newest message already delivered and the run names how many conversations are still open; ordinary capture delivers those once quiet. Load a reviewed iPhone backup or export when older, deleted, or attachment-only history matters.",
