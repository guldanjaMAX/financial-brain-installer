 ...
 *
 * The second half is the message an operator meets FIRST in that state. When
 * Cloudflare access cannot be established, the AUTH_REQUIRED failure used to
 * open with "Run `brain setup <manifest>`". Printed over a paused brain that is
 * the exact instruction that must never be followed, so the copy points at
 * signing in and re-running the SAME command instead.
 */
...
function installedManifest() {
  ...
  m.brain.domain = "riverbend-brain.owner-subdomain.workers.dev";
...
/** The /health shape a lease-protocol Worker serves, in either drain mode. */
const healthBody = (drainMode) => JSON.stringify({
  ok: true,
  version: PRODUCT_VERSION,
  vector_writer_protocol: "lease-v1",
  vector_drain_mode: drainMode,
  accepting_documents: drainMode === "active",
});
const asResponse = (body) => ({ ok: true, status: 200, text: async () => body });
...
try {
  ...
  const readDrainMode = async (drainMode) =>
    typeof brain.probeExistingWorkerDrainMode === "function"
      ? brain.probeExistingWorkerDrainMode(manifestPath, { http: async () => asResponse(healthBody(drainMode)) })
      : "<no probe>";
  ...
  const pausedAsFinished = await brain.probeExistingWorkerHealth(manifestPath, {
    http: async () => asResponse(healthBody("paused-for-upgrade")),
  });
  ...
  /* ------------------------------------------------------------------------
   * 2. setup REFUSES a paused brain
   * --------------------------------------------------------------------- */
  ...
  try {
    await quiet(() => brain.cmdSetup(manifestPath, setupOptions({
      // A paused brain is exactly what the finished-brain probe reports as null.
      probeExistingWorkerHealth: async () => null,
      probeExistingWorkerDrainMode: async () => "paused-for-upgrade",
    }, pausedTouched)));
  ...
  check("the refusal never sends the operator back to setup or drain",
    ...
    refusalMessage);
  check("and nothing that would pause, migrate or redeploy the brain ran",
    !pausedTouched.includes("bookmark") && !pausedTouched.includes("deploy") &&
      !pausedTouched.includes("migrate") && !pausedTouched.includes("wait") &&
      !pausedTouched.includes("drain"),
  ...
  try {
    await quiet(() => brain.cmdSetup(manifestPath, setupOptions({
      probeExistingWorkerHealth: async () => ({ version: PRODUCT_VERSION, acceptingDocuments: true }),
      probeExistingWorkerDrainMode: async () => { drainModeConsulted = true; return "active"; },
    }, activeTouched)));
  ...
  /* ------------------------------------------------------------------------
   * 4. the AUTH_REQUIRED copy no longer leads with setup
   * --------------------------------------------------------------------- */
  ...
  check("AUTH_REQUIRED no longer offers `brain setup` as its first suggestion",
    !/brain setup/.test(authMessage), authMessage);
  check("AUTH_REQUIRED tells the operator to sign in and re-run the SAME command",
    /sign in/i.test(authMessage) && /same command/i.test(authMessage), authMessage);
} finally {
