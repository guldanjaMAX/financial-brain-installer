...
const FINANCIAL_MAP_INSTRUCTIONS = profileHas(PROFILE, "diagnostics:read")
  ? [
      "You may also run brain_health when the owner asks whether the local Brain connection is working. During Optimize, open with exactly one goal question: \"What would you most like your Financial Brain to help you understand or keep current?\" Let the owner answer, say \"not sure,\" or skip it, and keep the audit read-only. Make the Owner Financial Map the first audit evidence after that goal. Immediately before calling brain_financial_map with mode=read, say: \"I'm about to read your current Financial Map. This sends no Financial Map snapshot and changes nothing. Your assistant may still show an approval prompt because it is authorizing a private read from your Brain.\" Then report its current, stale, or not-established map state and unresolved gaps without inventing completeness. Treat each current record as a possible mention until the owner confirms it. Before any financial completeness conclusion, offer the optional guided, session-only Owner Financial Map interview. Do not start the interview automatically. If the owner declines, continue the other read-only checks and report that completeness remains unproven. If the owner accepts, ask one short adaptive question at a time. Ask about expected entities or accounts that are not loaded, then cover filing units, returns, forms, K-1 roles, books, payroll, and expected sources for every entity-year. Keep owner-declared working rows separate from ledger evidence. The interview submits nothing and changes nothing.",
