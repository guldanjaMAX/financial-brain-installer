// The migration walk a real client brain has to survive: schema 22 -> 42.
//
...
// with a POPULATED database. Migrations 0023..0042 have
...
check("today's tree has 0023..0042 pending", PENDING.length === 20 && PENDING.at(-1).version === 42,
...
console.log("\n--- the walk: apply 0023..0042 to that populated brain ---");
const applied = [];
const errors = [];
...
check("all 20 pending migrations apply to a populated schema-22 brain",
...
if (errors.length === 0) {
  ...
  console.log("\n--- every declared object of 0023..0042 exists after the walk ---");
  ...
  for (const migration of PENDING) {
    ...
    for (const statement of splitStatements(migration.sql)) {
      ...
      if (object) { declared.push({ kind: "object", name: object[2] }); continue; }
      const column = statement.match(
        /^\s*ALTER\s+TABLE\s+([A-Za-z_][A-Za-z0-9_]*)\s+ADD\s+COLUMN\s+([A-Za-z_][A-Za-z0-9_]*)/i);
      if (column) declared.push({ kind: "column", table: column[1], name: column[2] });
    }
  ...
  const upsertError = probe(() => {
    ...
    ).run("fixture-brain", "0.4.0", 42, 0, new Date().toISOString(), "stable");
    return null;
  }, "threw");
  ...
  check("migrate records schema_version 42", state?.schema_version === 42, JSON.stringify(state?.schema_version));
  ...
  check("schema_migrations is contiguous 1..42 after the upgrade",
    versions.length === 42 && versions.every((v, i) => v === i + 1), JSON.stringify(versions));
}
...
console.log("\n--- 0033's DROP/CREATE window: the hazard the writer barrier exists to close ---");
{
  ...
  check("cmdMigrate lists 33 among the migrations requiring verified writer quiescence",
    /writerQuiescenceMigrations = new Set\(\[10, 11, 12, 13, 33\]\)/
...
// WHOLE file on the next `brain migrate`. Every statement in 0023..0042 must
