...
| A watched folder on your own machine | **Built, Mac-only for the schedule.** An `ongoing` folder reloads itself and keeps guarded mirror deletion. A `staging` or `one-time-import` folder can move only currently provenance-and-vector-confirmed local copies to system Trash through `brain cleanup-local`; every eligible file needs a keep, archive, or remove choice because outside-path validation is not custody through Trash, and retirement stops later walks without removing Brain documents. On Windows and Linux the same load and cleanup run by hand. Real multi-tick sleep, wake, retention, Trash recovery, and deletion behavior is not yet field-proven |
...
```json
"corpora": {
  "local_folder": { "enabled": true, "path": "/Users/you/Brain Dropbox", "source": "documents", "role": "ongoing" }
},
...
- **In an ongoing folder, a file you deleted is removed from your brain.** That is what makes the folder a mirror of your intent rather than a pile that only ever grows. Removals are counted and capped: a run that would remove more than 100 documents, or more than a tenth of what that source loaded, stops and asks you first, printing counts and an approval code but never a filename. A cloud folder that failed to sync looks exactly like you deleting everything in it, and that difference is worth one deliberate confirmation.
- **In a staging folder, a confirmed consumed file may leave this computer without removing its Brain document.** Run `brain cleanup-local <manifest>` first. It reports whether another current local path was observed, but treats every eligible file as a possible only copy at apply time and requires keep, archive, or remove because that outside path is not held in custody through Trash. It uses the system Trash rather than permanent deletion. `retention_days` applies that same proof after later successful scheduled loads with automatic keep or remove. A `one-time-import` source can be retired after completion so future loads stop walking it while its Brain documents stay.
