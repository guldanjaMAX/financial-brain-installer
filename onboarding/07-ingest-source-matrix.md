...
- **A full run can prove its own history, and says exactly what it proved.** A capture started with `--reset` and no `--limit` walks this Mac's whole message database from its first row to its last, and records that as a completed history sweep. What the claim covers is this Mac's database end to end and nothing wider: messages you deleted, messages that only ever lived on the phone, and attachment-only content sit outside it, which is what the iPhone backup load below is for. Tapbacks and attachment-only rows do not block the claim — they are read, counted and reported — but a row the walk cannot place in time at all does, and that count is named too. **That full run also closes and sends any conversation still in progress**, rather than leaving it waiting on the six-hour quiet gap: a sweep that claimed to cover this morning's thread while the brain could not yet return it would be the same missing record the claim exists to rule out. The run report says how many conversations it closed early, and the only visible effect is that one of those threads is split into two documents at the moment of the sweep. Ordinary capture ticks do not do this — they claim nothing, so they keep the plain six-hour rule. Until a full run like that has happened, `brain check` calls every category provisional rather than presenting unproven history as proven.
