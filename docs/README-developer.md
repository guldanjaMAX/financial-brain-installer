...
Gmail, Calendar, and IMAP use the parallel source lane:

```bash
brain schedule ./brain.manifest.json --install --source gmail
brain schedule ./brain.manifest.json --status --source calendar
brain schedule ./brain.manifest.json --remove --source imap
```

Their effective schedules come from `operations.source_crons`; the template
defaults place them at different minutes of each hour. Gmail and Calendar carry
only `BRAIN_GOOGLE_TOKEN_STORE` into the sparse child environment. IMAP carries
only `BRAIN_IMAP_CREDENTIAL_STORE`. Neither value is a credential, and the
installed definition never contains a password, token, admin key, or deployment
