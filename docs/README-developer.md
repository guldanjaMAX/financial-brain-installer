...
estimated vector and text-character savings, owner loss, and a plain-language
rule. Near duplicates and heavy records are candidates, not removal authority.
Titles are absent unless `include_sample_titles:true` is requested.

`POST /api/admin/brain/cleanup/plan` builds one exact rule plan. `POST
/api/admin/brain/cleanup/apply` only rebuilds that plan, proves the existing
forget dry run, and then refuses confirmed removal with
`cleanup_apply_unavailable`. Any target, content hash, chunk count, or selected
rule drift refuses with `cleanup_plan_changed`. Exact duplicate plans require
the same source, date, folder, client, category, entity, and platform boundary.
Their private response retains only four bounded source-location fields and
redacts titles by default. It does not persist those references or project them
into search results, citations, or the model prompt. Report, plan, and disabled
apply-preview routes refuse while a source is indexing, an update pause is
active, or the vector drain lease is live.
...
fingerprint, writes atomically, and performs exact readback. Cleanup removal
receipts do not exist in this version because no cleanup removal is applied.
The plan reports expected document, chunk/vector, text-character, and D1-size
effects only. `undo.supported` is false because the preview changes no stored
document and there is nothing to restore.
