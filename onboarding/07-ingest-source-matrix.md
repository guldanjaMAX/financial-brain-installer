...
| A watched folder on your own machine | **Built, Mac-only for the schedule.** An `ongoing` folder reloads itself and keeps guarded mirror deletion. A `staging` or `one-time-import` folder can move only currently provenance-and-vector-confirmed local copies to system Trash through `brain cleanup-local`; possible only copies need an extra choice, and retirement stops later walks without removing Brain documents. On Windows and Linux the same load and cleanup run by hand. Real multi-tick sleep, wake, retention, Trash recovery, and deletion behavior is not yet field-proven |
...
- **In a staging folder, a confirmed consumed file may leave this computer without removing its Brain document.** Run `brain cleanup-local <manifest>` first. It separates copies whose original was observed in another current local folder from possible only copies, requires the extra keep or remove choice for the latter, and uses the system Trash rather than permanent deletion. `retention_days` applies that same proof after later successful scheduled loads. A `one-time-import` source can be retired after completion so future loads stop walking it while its Brain documents stay.
