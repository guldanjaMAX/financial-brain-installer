...
// one) is at schema 22 with a POPULATED database. Migrations 0023..0035 have
...
check("today's tree has 0023..0035 pending", PENDING.length === 13 && PENDING.at(-1).version === 35,
...
console.log("\n--- the walk: apply 0023..0035 to that populated brain ---");
const applied = [];
const errors = [];
...
if (errors.length === 0) {
  ...
  console.log("\n--- every declared object of 0023..0035 exists after the walk ---");
  ...
  check("migrate records schema_version 35", state?.schema_version === 35, JSON.stringify(state?.schema_version));
  ...
  check("schema_migrations is contiguous 1..35 after the upgrade",
    versions.length === 35 && versions.every((v, i) => v === i + 1), JSON.stringify(versions));
}
...
// WHOLE file on the next `brain migrate`. Every statement in 0023..0035 must
