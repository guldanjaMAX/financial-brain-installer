...
- **Gmail, Calendar, and IMAP now have packaged unattended schedules.** After
  connecting the source, install it with `brain schedule <manifest> --install
  --source <gmail|calendar|imap>`. Status and remove are symmetric. macOS uses
  the same hardened per-user LaunchAgent machinery as Drive; Windows uses the
  current-user Task Scheduler adapter. Each source keeps its own cadence and
