...
test("one-target repair contract rehearses schema-44 admission and recovery end to end", async (t) => {
  ...
  const privatePlan = bindPrivateProvenanceTargetRepairSeal(draft, seal);
  ...
  const sealedOriginalId = privatePlan.seal.targets[0].original_id;
  const discoveryRequest = {
    ...
    targets: [{
      ...acceptedTarget(sealedOriginalId, null),
      observation_stage: "discovery",
      outcome: "gap",
      reason_code: "provenance_unassessed",
    }],
  ...
  const selectedDiscovery = selectPrivateProvenanceTargetDiscoveryReceipt(
    privatePlan,
    discovery,
  );
  assert.equal(selectedDiscovery.outcome, "gap");
  const discoveryReceipt = discovery;

  // This direct insert is a synthetic raw-bound family fixture. It represents
  // no reingest executor or authorization and keeps OCR disabled and unattempted.
  await seedBoundAcceptedFamily(fixture, sealedOriginalId);
  const acceptedFixtureRequest = (operation) => ({
    contract_version: 1,
    mode: "accepted_resolution",
    operation,
    source: SOURCE,
    ...
    targets: [acceptedTarget(
      sealedOriginalId,
      selectedDiscovery.observation_hash,
    )],
    retrieval_query: QUERY,
  });
  const prematureResponse = await fixture.post(
    SOURCE_ORIGINAL_OBSERVATION_PATH,
    acceptedFixtureRequest("record"),
    ADMIN,
  );
  ...
  ).n, 0, "accepted resolution cannot manufacture the portable schema-44 receipt");
  assert.deepEqual(acceptedCounts(fixture), {
    admissions: 0,
    resolutions: 0,
    activations: 0,
  ...
  const resultFamilyFixtureRequest = (operation) => ({
    contract_version: 1,
    mode: "result_family",
    operation,
    source: SOURCE,
  ...
  const familyRecordResponse = await fixture.post(
    SOURCE_ORIGINAL_OBSERVATION_PATH,
    resultFamilyFixtureRequest("record"),
    ADMIN,
  );
  const familyRecordBody = await body(familyRecordResponse);
  assert.equal(familyRecordResponse.status, 200, JSON.stringify(familyRecordBody));
  const familyRecord = validatePrivateProvenanceResultFamilyResponse(
    privatePlan,
    familyRecordBody,
    { operation: "record" },
  );
  ...
  const familyVerifyResponse = await fixture.post(
    SOURCE_ORIGINAL_OBSERVATION_PATH,
    resultFamilyFixtureRequest("verify"),
    ADMIN,
  );
  const familyVerifyBody = await body(familyVerifyResponse);
  assert.equal(familyVerifyResponse.status, 200, JSON.stringify(familyVerifyBody));
  const familyVerify = validatePrivateProvenanceResultFamilyResponse(
    privatePlan,
    familyVerifyBody,
    { operation: "verify", recordReceipt: familyRecord },
  );
  assert.equal(familyVerify.accepted_outcome_authorized, false);

  const acceptedValidation = {
    discoveryReceipt,
    resultFamilyReceipt: familyRecord,
  };
  const acceptedResponse = await fixture.post(
    SOURCE_ORIGINAL_OBSERVATION_PATH,
    acceptedFixtureRequest("record"),
  ...
  const acceptedBody = await body(acceptedResponse);
  assert.equal(acceptedResponse.status, 200, JSON.stringify(acceptedBody));
  const accepted = validatePrivateProvenanceAcceptedResolutionResponse(
    privatePlan,
    acceptedBody,
    {
      operation: "record",
  ...
  assert.equal(accepted.resolves_observation_hash, selectedDiscovery.observation_hash);
  ...
  const replayBody = await body(replayResponse);
  assert.equal(replayResponse.status, 200, JSON.stringify(replayBody));
  const replayed = validatePrivateProvenanceAcceptedResolutionResponse(
    privatePlan,
    replayBody,
    {
      operation: "record",
  ...
  assert.equal(replayed.resolves_observation_hash, selectedDiscovery.observation_hash);
  ...
  const verifyResponse = await fixture.post(
    SOURCE_ORIGINAL_OBSERVATION_PATH,
    acceptedFixtureRequest("verify"),
    ADMIN,
  );
  const verifyBody = await body(verifyResponse);
  assert.equal(verifyResponse.status, 200, JSON.stringify(verifyBody));
  const verified = validatePrivateProvenanceAcceptedResolutionResponse(
    privatePlan,
    verifyBody,
    {
      ...verifyOptions,
  ...
  assert.equal(verified.status, "accepted_resolution_current");
  assert.equal(verified.resolves_observation_hash, selectedDiscovery.observation_hash);
  ...
  const reactivateBody = await body(reactivateResponse);
  assert.equal(reactivateResponse.status, 200, JSON.stringify(reactivateBody));
  const reactivated = validatePrivateProvenanceAcceptedResolutionResponse(
    privatePlan,
    reactivateBody,
    {
      operation: "record",
  ...
  assert.equal(reactivated.status, "accepted_resolution_current");
  assert.equal(reactivated.resolves_observation_hash, selectedDiscovery.observation_hash);
  assert.equal(reactivated.reactivated, true);
  ...
  const finalVerifyBody = await body(finalVerifyResponse);
  assert.equal(finalVerifyResponse.status, 200, JSON.stringify(finalVerifyBody));
  const finalVerify = validatePrivateProvenanceAcceptedResolutionResponse(
    privatePlan,
    finalVerifyBody,
    {
      ...verifyOptions,
      recordReceipt: reactivated,
    },
  );
  assert.equal(finalVerify.status, "accepted_resolution_current");
  assert.equal(finalVerify.resolves_observation_hash, selectedDiscovery.observation_hash);
