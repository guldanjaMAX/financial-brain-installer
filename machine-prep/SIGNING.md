...
`.github/workflows/installer-signing.yml` is the separate, manually dispatched signing path. It never publishes a release. Each dispatch signs one target, `macos` or `windows`, and takes three values from the summary of a reviewed `machine-prep-installers` run: `unsigned_run_id`, `unsigned_artifact_id`, and `unsigned_sha256`. Before anything is downloaded, the job's first step reads GitHub's own records for that run and artifact and refuses unless the run came from `.github/workflows/machine-prep-installers.yml`, was started by `workflow_dispatch` on `main` of this repository (not a fork), finished with `success`, and uploaded exactly `FinancialBrainMachinePrep-unsigned.pkg` or `FinancialBrainMachinePrep-unsigned.msi` with that SHA-256 digest. The download then selects the artifact by its immutable ID with digest checking on, and the job hashes the downloaded bytes again before signing. It uploads a separately named signed artifact with a SHA-256 receipt that `shasum -a 256 -c` verifies from the extracted artifact. Both jobs run only in the protected `installer-signing` GitHub Environment.
...
4. Dispatch `machine-prep-installers` on `main`, review its artifacts, then copy the run ID, artifact ID, and SHA-256 from its summary into `installer-signing`.
