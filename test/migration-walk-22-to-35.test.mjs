...
// with a POPULATED database. Migrations 0023..0047 have
...
function applyMigration(db, migration, { onStatement = null } = {}) {
  ...
  return runRestartSafeMigrationStatements(
    ...
    (statement) => {
      ...
      // same way, which is what the column-existence probe consumes. D1 write
      // receipts also carry exact statement changes; the schema-47 page loop
      // must not infer completion without that receipt.
      const rows = db.prepare(statement).all();
      const changes = Number(db.prepare("SELECT changes() AS count").get().count);
...
check("today's tree has 0023..0047 pending", PENDING.length === 25 && PENDING.at(-1).version === 47,
...
console.log("\n--- the walk: apply 0023..0047 to that populated brain ---");
const applied = [];
const errors = [];
...
if (errors.length === 0) {
  ...
  console.log("\n--- every final declared object of 0023..0047 exists after the walk ---");
  ...
  const upsertError = probe(() => {
    ...
    ).run("fixture-brain", "0.4.0", 47, 0, new Date().toISOString(), "stable");
    return null;
  }, "threw");
  ...
  check("migrate records schema_version 47", state?.schema_version === 47, JSON.stringify(state?.schema_version));
  ...
  check("schema_migrations is contiguous 1..47 after the upgrade",
    versions.length === 47 && versions.every((v, i) => v === i + 1), JSON.stringify(versions));
}
...
console.log("\n--- 0033's DROP/CREATE window: the hazard the writer barrier exists to close ---");
{
  ...
  check("cmdMigrate lists 33, 44, 46, and 47 among the migrations requiring verified writer quiescence",
    /writerQuiescenceMigrations = new Set\(\[10, 11, 12, 13, 33, 44, 46, 47\]\)/
...
// WHOLE file on the next `brain migrate`. Every statement in 0023..0047 must
