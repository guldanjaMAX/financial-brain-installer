...
`.github/workflows/installer-signing.yml` is the separate, manually dispatched signing path. It never publishes a release. Each dispatch signs one target, `macos` or `windows`, and takes three values from the summary of a reviewed `machine-prep-installers` run: `unsigned_run_id`, `unsigned_artifact_id`, and `unsigned_sha256`. Before anything is downloaded, the job's first step reads GitHub's own records for that run and artifact and refuses unless the run came from `.github/workflows/machine-prep-installers.yml`, was started by `workflow_dispatch` on `main` of this repository (not a fork), built the commit at the tip of the `main` branch or an ancestor of it (proven through the compare API, with no tag named `main` in the repository), finished its one platform job (`build unsigned macOS package` or `build unsigned Windows MSI`) with `success`, and uploaded exactly `FinancialBrainMachinePrep-unsigned.pkg` or `FinancialBrainMachinePrep-unsigned.msi` with that SHA-256 digest. The download then selects the artifact by its immutable ID with digest checking on, and the job hashes the downloaded bytes again before signing. It uploads a separately named signed artifact with a SHA-256 receipt that `shasum -a 256 -c` verifies from the extracted artifact. Both jobs run only in the protected `installer-signing` GitHub Environment.

- macOS job: environment secrets `APPLE_DEVID_INSTALLER_P12_BASE64`, `APPLE_DEVID_INSTALLER_P12_PASSWORD`, `APPLE_NOTARY_KEY_P8`, `APPLE_NOTARY_KEY_ID`, and `APPLE_NOTARY_ISSUER_ID`, plus the repository variable `APPLE_TEAM_ID` (the ten-character Apple Developer Team ID). It imports the identity into a temporary keychain, signs only with the one Developer ID Installer identity of that team, runs `productsign --timestamp`, requires `pkgutil --check-signature` to name that team's identity, runs `xcrun notarytool submit --wait`, reads the notary log, staples, validates, runs `spctl -a -vv -t install`, and always deletes the keychain.
- Windows job: GitHub OIDC with Azure Artifact Signing. Variables (not secrets) `ARTIFACT_SIGNING_ENDPOINT`, `ARTIFACT_SIGNING_ACCOUNT_NAME`, `ARTIFACT_SIGNING_CERTIFICATE_PROFILE_NAME`, `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, and `AZURE_SUBSCRIPTION_ID`. It signs with SHA-256 and the `http://timestamp.acs.microsoft.com` timestamp server with the action's shared-cache restore turned off, then requires `Get-AuthenticodeSignature` to report Valid with a timestamp and a signer certificate whose subject has `O=Financial Brain LLC`, and `signtool verify /pa /v` to pass.
...
These identifiers are not secrets. They stay out of the workflow source and are set as repository variables:

- `ARTIFACT_SIGNING_ACCOUNT_NAME` repository variable: `financialbrain-signing` (region East US).
- `ARTIFACT_SIGNING_ENDPOINT` repository variable: `https://eus.codesigning.azure.net/`, the East US regional endpoint.
- `ARTIFACT_SIGNING_CERTIFICATE_PROFILE_NAME` repository variable: set only after the certificate profile is created. The certificate profile does not exist yet because identity validation is still pending, so the Windows job refuses as "not configured yet" until then.

## Owner checklist before the first signing run

1. In Settings > Environments > `installer-signing`, add the required reviewers, turn on **Prevent self-review** whenever more than one reviewer is listed, so the person who dispatches cannot approve their own run, and set Deployment branches and tags to **Selected branches** with a deployment branch policy of `main` only.
2. Bind the Azure federated credential to the environment, not a branch or pull request: issuer `https://token.actions.githubusercontent.com`, subject `repo:<owner>/<repo>:environment:installer-signing`, audience `api://AzureADTokenExchange`. Grant that identity only the Artifact Signing Certificate Profile Signer role on the profile.
3. Set the repository variables above and `APPLE_TEAM_ID`, and the Apple secrets in the `installer-signing` environment.
4. Dispatch `machine-prep-installers` on `main` (the branch; the signing gate refuses while a tag named `main` exists), review its artifacts, then copy the run ID, artifact ID, and SHA-256 from its summary into `installer-signing`. The signing gate checks the conclusion of the one platform job that built the target, `build unsigned macOS package` or `build unsigned Windows MSI`, through the run's jobs record, not the conclusion of the whole run. A default dispatch without `wix_osmf_confirmed=true` stops the Windows job on purpose, so the whole run shows as failed while its macOS package is still signable; the Windows MSI is signable only from a run whose Windows job succeeded.
