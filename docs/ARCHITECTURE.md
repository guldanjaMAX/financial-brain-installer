...
| Custom business API | A declarative `corpora.custom_api` manifest block becomes a plain Worker binding containing only endpoint rules and the name of a bearer-token secret. The value is entered through `brain connect custom-api` and remains a Worker secret. The existing Worker cron checks the D1 cadence lease before pulling. A byte-identical endpoint body skips row and document writes. Otherwise exact accepted rows and correction hashes are authoritative in D1, row-level numeric refusals remain counted, and readable documents use the ordinary document writer and vector outbox. Null store keys remain distinct as `unassigned`, missing expected categories are stated rather than converted to zero, and provider fields enter prose only when the manifest lists them. Missing upstream rows are retained. No custom API row is wired into the financial ledger or map. |
