...
const COMMAND = /\bbrain(?=\s+(?:init|setup|ask|financial-picture|machine-continuity|ocr-preflight|cleanup-local|doctor|whatsnew|verify|provision|deploy|secrets|health|test|mcp-config|assistant-repair|provenance-assess|provenance-repair|migrate|ingest|ingest-file|import|load|connect|disconnect|status|sources|forget|drain|reindex|diagnose|check|eval|grant|grants|zone|invite|devices|token|update|upgrade|rollback|schedule|support|tools|technician|--version)\b)/g;
/**
