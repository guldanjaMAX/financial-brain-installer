...
| Plaid | Built behind held field gate; general invitations closed | SQLite, scripted provider I/O, and offline atomic application-secret ceremony fixtures | None | Use the native connector only on the named disposable candidate. Application setup requires one nominated supervised owner computer because no remote first-setup compare-and-swap exists. Windows secret entry and custom-provider credential setup remain held. Prove owner passkey sign-in and private Link, explicit masked-account assignment with unassigned accounts staged, D1 promotion, cursor resume, real signed webhook and scheduled fallback, repair, provider-confirmed disconnect, and interruption recovery. Then complete the separately approved production pilot. |
