...
const CATALOG = [
  ...
  entry("INDEX_WRITE_FAILED", "Search indexing did not finish", "The document store accepted work that the search index could not yet confirm.", "The durable queue keeps the unfinished search work available for recovery.", "review_first", ["Run brain health with the same manifest and read its current status.", "When status is ok and accepting documents, run brain drain. When status is paused for an update, resume brain update first."], "The queue does not shrink across two active-state drain attempts, or the update cannot resume."),
  ...
  entry("VECTOR_DRAIN_FAILED", "Meaning search is still catching up", "The durable search queue did not reach zero or confirm visibility in time.", "Keyword search and stored documents remain separate from unconfirmed vector work, and the gap stays explicit.", "review_first", ["Run brain health with the same manifest and read its current status.", "When status is ok and accepting documents, resume brain drain. When status is paused for an update, resume brain update first."], "The remaining count does not fall during two active-state drain attempts, or the update cannot resume."),
];
