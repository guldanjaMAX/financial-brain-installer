...
function makeEnv({ vectorCount = null, drainMode = null, beforeStatement = null } = {}) {
...
/* ---- total D1 observability failure can never masquerade as an empty brain ---- */
{
  ...
  check("an incomplete diagnosis renders unverified counts and no false-green assurance",
    /not verified\s+documents/.test(rendered) && /diagnosis is incomplete/i.test(rendered) &&
