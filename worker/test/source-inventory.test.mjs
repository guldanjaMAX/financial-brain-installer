...
function addInventoryFixture(db, prefix = "") {
  ...
  insertDocument.run(
    ...
    JSON.stringify({ evidence_lineage: { version: 1, kind: "source_record", root_ids: [] } }),
    "native", 1,
  );
  insertDocument.run(
    ...
    JSON.stringify({ family_of: `${source("alpha")}:two` }), "ocr_partial", 0,
  );
  insertDocument.run(
    `${source("beta")}:one`, source("beta"), "one", "Invented beta", null,
    Date.parse("2026-08-03T00:00:00.000Z"), "fixture", 1, null, null,
    Date.parse("2026-09-01T00:00:00.000Z"), "hash-beta-one", "{}", "ocr", 0,
  );
  insertDocument.run(
...
test("source inventory pages are complete, stable, supported, and read-only", async () => {
  ...
  assert.equal(first.recovery_plan_summary.reason_counts.derivation_lineage_missing, 1);
  assert.equal(first.recovery_plan_summary.reason_counts.lineage_contract_unrecognized, 1);
  ...
  assert.equal(first.sources[0].provenance.lineage.recognized_contract_documents, 1);
...
test("source recovery preview is exhaustive through stable opaque pages and performs no repair", async () => {
  ...
  assert.ok(candidates.every((candidate) => /^sha256:[a-f0-9]{64}$/.test(candidate.record_id)));
  ...
  assert.deepEqual(candidates[2].provenance.missing_subfields, ["recognized_lineage_contract"]);
  assert.deepEqual(candidates[2].reasons, ["lineage_contract_unrecognized"]);
