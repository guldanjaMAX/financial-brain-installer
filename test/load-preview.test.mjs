...
const candidates = [
  ...
  { rel: "imports/decode-failure.txt", name: "decode-failure.txt", size: 900 },
];
...
preview.observePrepared(candidates[3], {
  envelope: { content: "xq0 @@@ ### %%% ||| <> ".repeat(240) },
  quality_flags: [{ code: "mostly_symbols", reason: "the extraction is mostly symbols with too little readable text" }],
});
preview.observePrepared(candidates[4], {
  skip: { reason: "the text decoded into mostly unreadable characters (wrong or unsupported encoding)", metrics: { replacement_ratio: 0.91 } },
});
...
assert.equal(report.summary.files.total, 5);
assert.equal(report.summary.files.by_type[".txt"].count, 3);
assert.equal(report.summary.quality_refusals.total, 1);
assert.equal(report.summary.quality_review_flags.documents, 1);
assert.equal(report.summary.quality_review_flags.by_reason["mostly symbols with too little readable text"], 1);
...
try {
  ...
  detailedPreview.observePrepared(candidates[4], {
    skip: { reason: "the text decoded into mostly unreadable characters (wrong or unsupported encoding)", metrics: { replacement_ratio: 0.91 } },
  });
  detailedPreview.finish();
  const saved = JSON.parse(readFileSync(output, "utf8"));
  assert.equal(saved.contract_version, 1);
  assert.equal(saved.detail.files.length, 5);
  assert.equal(saved.detail.quality_review_flags.length, 1);
...
try {
  ...
  writeFileSync(join(root, "fixed-width-register.txt"), [
  ...
  assert.equal(result.load_preview.quality_refusals.total, 0);
  assert.equal(result.load_preview.quality_review_flags.documents, 2);
  assert.equal(result.would_send, 4,
    "the local fixed-width control and heuristic-flag control did not reach the prepared/send decision");
  ...
  assert.equal(cliDetail.detail.files.length, 4);
  ...
  assert.equal(defaultResult.load_preview.quality_refusals.total, 0);
  assert.equal(defaultResult.load_preview.quality_review_flags.documents, 1,
    "the default CLI path did not reach the quality-review decision");
  assert.equal(defaultResult.would_send, 1,
    "a heuristic quality flag blocked the prepared/send decision");
  ...
  assert.equal(overrideResult.load_preview.quality_review_flags.documents, 0);
