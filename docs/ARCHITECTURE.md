...
| Custom business API | A declarative `corpora.custom_api` manifest block becomes a plain Worker binding containing only endpoint rules and the name of a bearer-token secret. The value is entered through `brain connect custom-api` and remains a Worker secret. The existing Worker cron checks the D1 cadence lease before pulling. Exact rows and correction hashes are authoritative in D1; readable documents use the ordinary document writer and vector outbox. Missing upstream rows are retained. No custom API row is wired into the financial ledger or map. |
