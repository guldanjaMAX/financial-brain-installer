// The migration walk a real client brain has to survive: schema 22 -> 49.
//
...
// with a POPULATED database. Migrations 0023..0049 have
...
check("today's tree has 0023..0049 pending", PENDING.length === 27 && PENDING.at(-1).version === 49,
...
console.log("\n--- the walk: apply 0023..0049 to that populated brain ---");
const applied = [];
const errors = [];
...
check("all 27 pending migrations apply to a populated schema-22 brain",
...
if (errors.length === 0) {
  ...
  console.log("\n--- every final declared object of 0023..0049 exists after the walk ---");
  ...
  const upsertError = probe(() => {
    ...
    ).run("fixture-brain", "0.4.0", 49, 0, new Date().toISOString(), "stable");
    return null;
  }, "threw");
  ...
  check("migrate records schema_version 49", state?.schema_version === 49, JSON.stringify(state?.schema_version));
  ...
  check("schema_migrations is contiguous 1..49 after the upgrade",
    versions.length === 49 && versions.every((v, i) => v === i + 1), JSON.stringify(versions));
}
...
// WHOLE file on the next `brain migrate`. Every statement in 0023..0049 must
